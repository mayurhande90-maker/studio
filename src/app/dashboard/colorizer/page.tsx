...
import { ArrowRight, Download, Sparkles, Loader2, UploadCloud, RefreshCw, Palette, Upload } from 'lucide-react';
...
const generationMessages = [
    ...
    { text: "Finding the right colors...", percentage: 30 },
    { text: "Painting with AI...", percentage: 60 },
    { text: "Adding realistic tones...", percentage: 80 },
    { text: "Finalizing your memory...", percentage: 95 },
];
    ...
    const handleAnalysis = useCallback(async () => {
        ...
        try {
            ...
            setAnalysisResult({
                ...
                subject: 'Unknown',
                friendlyCaption: 'There was an issue analyzing this image. You can still try to colorize.'
            });
        } finally {
        ...
        <div className="w-full space-y-8">
            <div className="text-center">
                ...
                <p className="text-lg text-muted-foreground mt-2 max-w-2xl mx-auto">
                    Breathe new life into your old black & white photos with a touch of AI magic.
                </p>
            </div>
                    ...
                    <CardContent className="p-0 relative h-full flex items-center justify-center min-h-[450px]">
                             ...
                             <div className="w-full h-full grid grid-cols-2">
                                <div className="relative">
                                    <Image 
                                        ...
                                        objectFit="cover"
                                    />
                                    <div className="absolute bottom-2 left-2 bg-black/50 text-white text-xs font-bold p-1 rounded">ORIGINAL</div>
                                </div>
                                <div className="relative">
                                     {generatedImage ? (
                                        <Image 
                                     ...
                                     <div className="absolute bottom-2 right-2 bg-black/50 text-white text-xs font-bold p-1 rounded">COLORIZED</div>
                                </div>
                            </div>
                        )}
                        ...
                        {isGenerating && (
                            <div className="absolute inset-0 flex flex-col items-center justify-center bg-black/60 p-8 text-center">
                                <Palette className="w-16 h-16 text-primary animate-pulse" />
                                <p className="font-bold text-2xl text-primary-foreground mt-4">{generationMessages[generationStep].text}</p>
                                <p className="text-muted-foreground mt-2">Our AI is restoring your memory. Please wait a moment.</p>
                        ...
                        {!preview && (
                            ...
                            >
                                ...
                                <p className="text-xl font-bold">Drop your B&W or old photo here</p>
                                    ...
                                    {isAnalyzing ? (
                                        ...
                                        <>
                                            <p className="font-semibold text-foreground">
                                                {analysisResult ? analysisResult.friendlyCaption : 'Upload a photo to see what our AI thinks.'}
                                            </p>
                                            {analysisResult && (
                                            <div className="text-xs text-muted-foreground">
                                                <span>Detected: <strong>{analysisResult.subject}</strong></span> | <span>B&W: <strong>{analysisResult.isBlackAndWhite ? 'Yes' : 'No'}</strong></span>
                                            </div>
                                            )}
                                        </>
                                    )}
                        ...
                        <CardContent className="space-y-4">
                            {generatedImage && !isGenerating ? (
                                <div className="grid grid-cols-1 gap-4">
                                    ...
                                    <Button onClick={handleRegenerate} variant="outline" size="lg" className="w-full rounded-2xl">
                            ...
                            <>
                                <Button onClick={handleGenerate} variant="gradient" size="lg" className="w-full font-bold text-lg py-7 rounded-2xl hover:shadow-lg hover:shadow-primary/40" disabled={isGenerating || isAnalyzing || !analysisResult}>
                                    {isGenerating ? (
                                        ...
                                        <>
                                            Colorize Image <ArrowRight className="ml-2 h-5 w-5" />
                                        </>
                                    )}
                                </Button>
